...
Using WikiText Dataset from ModelScope
...
Dataset: WikiText-2 from ModelScope (魔塔社区)
...
class WikiTextBenchmark:
    """Comprehensive benchmark using WikiText dataset from ModelScope."""
    ...
    def _load_wikitext_prompts(self) -> List[str]:
        ...
        try:
            ...
            for item in dataset:
                ...
                if not text or text.strip() == '' or text.startswith(' ='):
                    ...
                    if current_text and len(current_text) >= self.min_prompt_length:
                        ...
                        current_text = ""
                        if len(prompts) >= self.num_samples * 2:  # Get extra in case we need to filter
                            break
                else:
                    current_text += " " + text
            ...
            for prompt in prompts:
                # Clean up and truncate to max_prompt_length
                prompt = prompt.strip()
                if len(prompt) >= self.min_prompt_length:
                    # Truncate to max_prompt_length characters for consistent benchmark
            ...
            if len(valid_prompts) < self.num_samples:
                print(f"Warning: Only found {len(valid_prompts)} valid prompts, needed {self.num_samples}")
                # Duplicate if needed
                while len(valid_prompts) < self.num_samples:
    ...
    def run_all_benchmarks(self):
        ...
        print("\n" + "#"*70)
        print("# Comprehensive Speculative Decoding Benchmark (WikiText)")
    ...
    def _generate_summary(self, acceptance_comparison: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Print summary table
        print("\n" + "="*110)
        print("FINAL RESULTS SUMMARY (WikiText Dataset)")
        print("="*110)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Comprehensive Speculative Decoding Benchmark (WikiText)")
    ...
    if args.output is None:
        ...
        args.output = f"results/wikitext_benchmark_{timestamp}.json"
