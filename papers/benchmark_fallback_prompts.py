...
Using Fallback Prompts (Fixed AI-related text prompts)

This script benchmarks multiple speculative decoding configurations using
fixed fallback prompts (the original implementation with repeated AI-related text).

Tested configurations:
1. Tree V2 (D=4/5/6/7, B=2, various thresholds)
...
class FLOPsEstimator:
    ...
    def estimate_ar_decode_flops(self, num_tokens: int, prompt_len: int) -> float:
        ...
        for i in range(num_tokens):
            total += self.target.estimate_forward_flops(1)
        return total
    
    def estimate_spec_decode_flops(self, num_tokens: int, K: int, num_rounds: int, prompt_len: int) -> float:
    ...
    def estimate_tree_decode_flops(self, num_tokens: int, depth: int, branch: int, num_rounds: int, prompt_len: int) -> float:
...
class FallbackPromptsBenchmark:
    """Benchmark using fixed fallback prompts (AI-related text)."""
    ...
    def _measure_prefill_ttft(self, prompt: str, model) -> float:
        ...
        torch.cuda.synchronize()
        return (time.perf_counter() - start_prefill) * 1000
    ...
    def benchmark_baseline(self) -> BenchmarkMetrics:
        ...
        all_ttft, all_tpot, all_throughput, all_tokens = [], [], [], []
        ...
        metrics.decode_flops = self.flops_estimator.estimate_ar_decode_flops(metrics.total_tokens_generated, avg_prompt_len)
        ...
        print(f"\nBaseline Results: Throughput: {metrics.throughput_tps:.1f} t/s, TTFT: {metrics.ttft_ms:.1f} ms, TPOT: {metrics.tpot_ms:.2f} ms")
    ...
    def benchmark_hf_assisted(self) -> BenchmarkMetrics:
        ...
        for run_idx in range(self.warmup_runs + len(self.prompts)):
            ...
            with self._memory_tracking():
                ...
                output_ids = self.target_model.generate(
                    input_ids, assistant_model=self.draft_model,
                    max_new_tokens=self.max_new_tokens, do_sample=False,
                    pad_token_id=self.tokenizer.pad_token_id, eos_token_id=999999,
                )
                torch.cuda.synchronize()
            ...
            if not is_warmup:
                ...
                all_ttft.append(ttft)
                all_tpot.append(tpot)
                print(f"  Sample {run_idx - self.warmup_runs + 1}: {throughput:.1f} t/s, tokens={num_tokens}")
        ...
        print(f"\nHF Assisted Results: Throughput: {metrics.throughput_tps:.1f} t/s, Speedup: {metrics.speedup:.2f}x")
    ...
    def benchmark_linear_spec_decode(self, K: int = 6) -> BenchmarkMetrics:
        ...
        generator = SpeculativeGenerator(
            target_model=self.target_model, draft_model=self.draft_model,
            tokenizer=self.tokenizer, K=K, device=self.device, use_compile=False
        )
        
        all_throughput, all_acceptance, all_tokens_per_round, all_rounds, all_ttft, all_tpot = [], [], [], [], [], []
        ...
        avg_prompt_len = 512
        metrics.prefill_flops = self.flops_estimator.estimate_prefill_flops(avg_prompt_len)
        metrics.decode_flops = self.flops_estimator.estimate_spec_decode_flops(metrics.total_tokens_generated, K, metrics.total_rounds, avg_prompt_len)
        metrics.total_flops = metrics.prefill_flops + metrics.decode_flops
        metrics.flops_per_token = metrics.decode_flops / metrics.total_tokens_generated
        ...
        print(f"\nLinear K={K} Results: Throughput: {metrics.throughput_tps:.1f} t/s, Speedup: {metrics.speedup:.2f}x")
    ...
    def benchmark_streaming_spec_decode(self, K: int = 6, max_cache_len: int = 1024) -> BenchmarkMetrics:
        ...
        metrics = BenchmarkMetrics(method=f"Streaming K={K} cache={max_cache_len}", config={"K": K, "max_cache_len": max_cache_len})
        original_eos = self._disable_eos()
        
        generator = StreamingSpeculativeGenerator(
            target_model=self.target_model, draft_model=self.draft_model,
            tokenizer=self.tokenizer, K=K, max_cache_len=max_cache_len,
            device=self.device, use_compile=False
        )
        
        all_throughput, all_acceptance, all_ttft, all_tpot = [], [], [], []
        ...
        print(f"\nStreaming K={K} Results: Throughput: {metrics.throughput_tps:.1f} t/s, Speedup: {metrics.speedup:.2f}x")
        
        self.results.append(metrics)
        return metrics
    
    @torch.inference_mode()
    def benchmark_tree_v2(self, tree_depth: int = 8, branch_factor: int = 3, probability_threshold: float = 0.03) -> BenchmarkMetrics:
        print("\n" + "="*60)
        ...
        metrics = BenchmarkMetrics(
            ...
            config={"tree_depth": tree_depth, "branch_factor": branch_factor, "probability_threshold": probability_threshold}
        )
        original_eos = self._disable_eos()
        
        generator = TreeSpeculativeGeneratorV2(
            target_model=self.target_model, draft_model=self.draft_model,
            tokenizer=self.tokenizer, tree_depth=tree_depth, branch_factor=branch_factor,
            probability_threshold=probability_threshold, device=self.device, use_compile=False
        )
        ...
        avg_prompt_len = 512
        metrics.prefill_flops = self.flops_estimator.estimate_prefill_flops(avg_prompt_len)
        metrics.decode_flops = self.flops_estimator.estimate_tree_decode_flops(metrics.total_tokens_generated, tree_depth, branch_factor, metrics.total_rounds, avg_prompt_len)
        metrics.total_flops = metrics.prefill_flops + metrics.decode_flops
        metrics.flops_per_token = metrics.decode_flops / metrics.total_tokens_generated
        ...
        print(f"\nTree V2 Results: Throughput: {metrics.throughput_tps:.1f} t/s, Speedup: {metrics.speedup:.2f}x")
        
        self.results.append(metrics)
        return metrics
    
    def compare_acceptance_rates(self) -> Dict[str, Any]:
        comparison = {"linear_methods": [], "tree_methods": [], "analysis": {}}
        
        for metrics in self.results:
            if metrics.method.startswith("Linear"):
                comparison["linear_methods"].append({
                    "method": metrics.method, "acceptance_rate": metrics.acceptance_rate,
                    "tokens_per_round": metrics.tokens_per_round, "total_rounds": metrics.total_rounds,
                })
            elif metrics.method.startswith("Tree"):
                comparison["tree_methods"].append({
                    "method": metrics.method, "acceptance_rate": metrics.acceptance_rate,
                    "avg_path_length": metrics.avg_path_length, "total_rounds": metrics.total_rounds,
                })
        
        if comparison["linear_methods"]:
            comparison["analysis"]["avg_linear_acceptance"] = sum(m["acceptance_rate"] for m in comparison["linear_methods"]) / len(comparison["linear_methods"])
            comparison["analysis"]["avg_linear_tokens_per_round"] = sum(m["tokens_per_round"] for m in comparison["linear_methods"]) / len(comparison["linear_methods"])
        
        if comparison["tree_methods"]:
            comparison["analysis"]["avg_tree_acceptance"] = sum(m["acceptance_rate"] for m in comparison["tree_methods"]) / len(comparison["tree_methods"])
            comparison["analysis"]["avg_tree_path_length"] = sum(m["avg_path_length"] for m in comparison["tree_methods"]) / len(comparison["tree_methods"])
        
        return comparison
    
    def run_all_benchmarks(self):
        print("\n" + "#"*70)
        print("# Comprehensive Speculative Decoding Benchmark (Fallback Prompts)")
        ...
        # 2. Tree V2 configurations (same as wikitext benchmark)
        tree_configs = [
            {"tree_depth": 4, "branch_factor": 2, "probability_threshold": 0.05},
            {"tree_depth": 4, "branch_factor": 2, "probability_threshold": 0.03},
            {"tree_depth": 5, "branch_factor": 2, "probability_threshold": 0.05},
            {"tree_depth": 7, "branch_factor": 2, "probability_threshold": 0.05},
            {"tree_depth": 6, "branch_factor": 2, "probability_threshold": 0.05},
        ]
    ...
    def _generate_summary(self, acceptance_comparison: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sorted_by_acceptance = sorted([m for m in self.results if m.acceptance_rate > 0], key=lambda x: x.acceptance_rate, reverse=True)
        ...
        print("\n" + "="*100)
        print("FINAL RESULTS SUMMARY (Fallback Prompts Dataset)")
        print("="*100)
        print(f"\n{'Rank':<5} {'Method':<35} {'Throughput':<12} {'Speedup':<10} {'Accept%':<10}")
        print("-"*100)
        for i, m in enumerate(sorted_by_throughput):
            accept_str = f"{m.acceptance_rate:.1%}" if m.acceptance_rate > 0 else "N/A"
            print(f"{i+1:<5} {m.method:<35} {m.throughput_tps:>8.1f} t/s {m.speedup:>7.2f}x {accept_str:>10}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark with Fallback Prompts")
    ...
    benchmark = FallbackPromptsBenchmark(
    ...
    if args.output is None:
        ...
        args.output = f"results/fallback_benchmark_{args.max_new_tokens}tokens.json"
