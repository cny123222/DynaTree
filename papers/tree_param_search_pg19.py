#!/usr/bin/env python3
"""
Tree-based Speculative Decoding Parameter Search using PG-19 Dataset

This script systematically searches for the optimal Tree V2 configuration
using prompts from PG-19 dataset (book excerpts).
...
Usage:
    python tree_param_search_pg19.py \
...
DEFAULT_PG19_PATH = "/mnt/disk1/ljm/LLM-Efficient-Reasoning/data/pg19.parquet"


# =============================================================================
# PG-19 Dataset Loading
# =============================================================================
def load_pg19_prompts(
    dataset_path: str = DEFAULT_PG19_PATH,
    num_prompts: int = 5,
    min_length: int = 200,
    max_length: int = 800
) -> List[str]:
    """
    Load prompts from PG-19 dataset (parquet file).
    
    Args:
        dataset_path: Path to pg19.parquet file
    ...
    """
    print(f"Loading PG-19 prompts from {dataset_path}...")
...
def search_tree_params(
    ...
    print(f"Estimated time: {format_time(total_configs * 5)}")  # ~5s per config
...
def plot_heatmaps(results: List[Dict], output_path: str):
    ...
    plt.suptitle('Tree V2 Parameter Search (PG-19): Speedup vs D and threshold', fontsize=14)
    plt.tight_layout()
...
def plot_speedup_by_tokens(results: List[Dict], output_path: str):
    ...
    ax.set_title('Best Tree V2 Speedup by Token Length (PG-19 Dataset)')
...
# =============================================================================
def main():
    parser = argparse.ArgumentParser(description="Tree V2 Parameter Search (PG-19 Dataset)")
    ...
    parser.add_argument("--dataset", type=str, default=DEFAULT_PG19_PATH,
    ...
    parser.add_argument("--num-prompts", type=int, default=5,
                       help="Number of prompts to use from PG-19")
    ...
    print("=" * 70)
    print("Tree V2 Parameter Search (PG-19 Dataset)")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("SEARCH RESULTS SUMMARY (PG-19 Dataset)")
    print("=" * 70)
    ...
    json_path = os.path.join(args.output_dir, f"tree_param_search_pg19_{timestamp}.json")
    with open(json_path, 'w') as f:
        json.dump(output_data, f, indent=2)
    print(f"\nResults saved to: {json_path}")
    
    # Generate visualizations
    heatmap_path = f"papers/figures/tree_param_heatmap_pg19_{timestamp}.png"
    plot_heatmaps(results, heatmap_path)
    
    speedup_path = f"papers/figures/tree_param_speedup_pg19_{timestamp}.png"
