#!/usr/bin/env python3
"""
Benchmark: Phase 2 Adaptive Tree Speculative Decoding

This script compares Phase 1 (adaptive branching) vs Phase 2 (adaptive branching + dynamic depth):
- Control: TreeSpeculativeGeneratorV2Adaptive (Phase 1)
- Experimental: TreeSpeculativeGeneratorV2AdaptiveV2 (Phase 2 - with dynamic depth)
...
@dataclass
class Phase2Metrics:
    """Metrics for Phase 2 dynamic depth comparison."""
    method: str
    config: Dict[str, Any]
...
class Phase2AdaptiveBenchmark:
    """Phase 2 benchmark: Phase 1 vs Phase 2 (with dynamic depth)."""
    ...
    def __init__(
        ...
        self.results: List[Phase2Metrics] = []
    ...
    @torch.inference_mode()
    def benchmark_phase1(self, base_depth: int) -> Phase2Metrics:
        """Benchmark Phase 1 (adaptive branching only)."""
        method_name = f"Phase1 (Adaptive Branch, D={base_depth})"
    ...
    def benchmark_phase2(
        ...
        """Benchmark Phase 2 (adaptive branching + dynamic depth)."""
        method_name = f"Phase2 (Dynamic Depth, D={base_depth}-{max_depth})"
        ...
        for run_idx in range(self.warmup_runs + len(self.prompts)):
            ...
            if not is_warmup:
                ...
                print(f"  Sample {run_idx - self.warmup_runs + 1}: {throughput:.1f} t/s, accept={stats.get('acceptance_rate', 0):.2%}, "
                      f"early_stop={depth_stats.get('early_stops', 0)}, deep_exp={depth_stats.get('deep_expansions', 0)}")
    ...
    def run_phase2_study(self):
        """Run Phase 2 comparison study."""
        print("\n" + "#"*80)
        print("# Phase 2: Dynamic Depth with Early Stopping")
        ...
        # 2. Compare Phase 1 vs Phase 2 for different base depths
        for base_depth in [4, 5]:
            # Phase 1: Adaptive branching only
            self.benchmark_phase1(base_depth=base_depth)
            
            # Phase 2: Adaptive branching + dynamic depth
            self.benchmark_phase2(
                base_depth=base_depth,
                max_depth=base_depth + 4,  # Allow extending up to 4 more levels
                early_stop_threshold=0.1,
                deep_expand_threshold=0.5,
            )
    
    def _generate_summary(self) -> Dict[str, Any]:
        """Generate summary of Phase 2 results."""
        summary = {
            "experiment_info": {
                "phase": "Phase 2",
                "focus": "dynamic_depth_early_stopping",
        ...
        # Print summary
        print("\n" + "="*100)
        print("PHASE 2 RESULTS: Phase 1 vs Phase 2 (Dynamic Depth)")
        print("="*100)
        print(f"\n{'Method':<45} {'Throughput':<12} {'Speedup':<10} {'Accept%':<10} {'EarlyStop':<10} {'DeepExp':<10}")
        print("-"*100)
        
        for m in self.results:
            accept_str = f"{m.acceptance_rate:.1%}" if m.acceptance_rate > 0 else "N/A"
            early_str = f"{m.early_stops}" if m.early_stops > 0 else "N/A"
            deep_str = f"{m.deep_expansions}" if m.deep_expansions > 0 else "N/A"
            print(f"{m.method:<45} {m.throughput_tps:>8.1f} t/s {m.speedup:>7.2f}x {accept_str:>10} {early_str:>10} {deep_str:>10}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Phase 2: Dynamic Depth Benchmark")
    ...
    parser.add_argument("--output", type=str, default="results/adaptive_phase2.json")
    ...
    benchmark = Phase2AdaptiveBenchmark(
