...
We evaluate how throughput varies with generation length \(T\) on WikiText-2, comparing autoregressive decoding, linear speculative decoding, a fixed tree baseline, and DynaTree. Figure~\ref{fig:length-scaling} summarizes throughput trends across \(T\) (left) and decomposes these trends via \(\bar{L}\), \(\bar{\ell}\), and acceptance rate (right), illustrating how per-iteration progress and verification success vary with generation length.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.49\linewidth]{../figures/length_scaling.pdf}\hfill
  \includegraphics[width=0.49\linewidth]{../figures/length_scaling_efficiency.pdf}
  \caption{\textbf{Sequence length scaling on WikiText-2.} \textbf{Left:} Throughput (tokens/s) as a function of \(T\) for AR, linear speculative decoding, a fixed tree baseline, and DynaTree. \textbf{Right:} Verification efficiency (\(\bar{L}\), \(\bar{\ell}\), and acceptance rate) as functions of \(T\).}
