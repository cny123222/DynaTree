...
\State \(\ell \gets \Call{SeqLen}{\mathcal{K}_T}\)
\State \(\mathcal{T} \gets \Call{DraftTree}{x_{1:t}, D, B, \tau, N_{\max}}\)
\State \((\mathbf{z}_{1:n}, \mathbf{A}) \gets \Call{FlattenMask}{\mathcal{T}, \ell}\)
\State \(\hat{\mathbf{y}} \gets \Call{VerifyTree}{\mathbf{z}_{1:n}, \mathbf{A}, \mathcal{K}_T}\)
\State \(y_{t+1:t+L} \gets \Call{SelectCommit}{\mathcal{T}, \hat{\mathbf{y}}, \mathbf{s}_{\text{last}}}\)
\State \(\mathcal{K}_T \gets \Call{UpdateCache}{\mathcal{K}_T, y_{t+1:t+L}, \ell}\)
\State \Return \(y_{t+1:t+L}\)

\Function{\textproc{DraftTree}}{$x_{1:t}, D, B, \tau, N_{\max}$}
  \Comment{Matches \texttt{TreeSpeculativeGeneratorV2}}
  \State Run \(M_D\) on \(x_{1:t}\); let \(u_0\) be the \(\top 1\) token; initialize tree \(\mathcal{T}\) with root \(u_0\)
  ...
  \For{$d=1$ \textbf{to} $D$}
    ...
    \ForAll{$u \in \mathcal{A}$}
      \If{$\bar{\ell}_u < \log \tau$} \State \textbf{continue} \EndIf \Comment{V2 pruning: skip expanding branch}
...
\Function{\textproc{FlattenMask}}{$\mathcal{T}, \ell$}
  \State \(\mathbf{z}_{1:n} \gets\) BFS order of nodes in \(\mathcal{T}\)
  \State \(\mathbf{A} \gets\) tree causal mask (all prefix positions + ancestors only)
  \State \Return \((\mathbf{z}_{1:n}, \mathbf{A})\)
\EndFunction

\Function{\textproc{VerifyTree}}{$\mathbf{z}_{1:n}, \mathbf{A}, \mathcal{K}_T$}
  \State \(\mathbf{s}_{1:n} \gets M_T(\mathbf{z}_{1:n}; \mathbf{A}, \mathcal{K}_T)\)
  \State \(\hat{\mathbf{y}} \gets \arg\max \mathbf{s}_{1:n}\) \Comment{greedy top-1 per node}
  \State \Return \(\hat{\mathbf{y}}\)
\EndFunction

\Function{\textproc{SelectCommit}}{$\mathcal{T}, \hat{\mathbf{y}}, \mathbf{s}_{\text{last}}$}
  \Comment{Matches \texttt{\_select\_best\_path}}
...
\Function{\textproc{UpdateCache}}{$\mathcal{K}_T, y_{t+1:t+L}, \ell$}
  \Comment{Matches \texttt{\_update\_tree\_cache}}
  \State \(\mathcal{K}_T \gets \Call{Crop}{\mathcal{K}_T,\ell}\)
  \State Run \(M_T\) on \(y_{t+1:t+L}\) with cache \(\mathcal{K}_T\) to rebuild \(\mathcal{K}_T\)
  \State \Return \(\mathcal{K}_T\)
\EndFunction
\end{algorithmic}
\end{algorithm}
