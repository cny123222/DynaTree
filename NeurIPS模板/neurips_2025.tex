...
\Function{\textproc{DraftTree}}{$x_{1:t}, D, B, \tau, N_{\max}$}
  \Comment{Draft model expands \(\top B\) per node to depth \(D\), prunes leaves with \(\bar{\ell}_u<\log\tau\), and enforces budget \(N_{\max}\).}
  \State \Return pruned token tree \(\mathcal{T}\)
\EndFunction

\Statex
\Function{\textproc{FlattenMask}}{$\mathcal{T}, \ell$}
  \Comment{Serialize \(\mathcal{T}\) in BFS order and build the tree causal mask (prefix + ancestors only).}
  \State \Return \((\mathbf{z}_{1:n}, \mathbf{A})\)
\EndFunction

\Statex
\Function{\textproc{VerifyTree}}{$\mathbf{z}_{1:n}, \mathbf{A}, \mathcal{K}_T$}
  \Comment{One target forward pass under \(\mathbf{A}\); return per-node greedy \(\top 1\) predictions \(\hat{\mathbf{y}}\).}
  \State \Return \(\hat{\mathbf{y}}\)
\EndFunction

\Statex
\Function{\textproc{SelectCommit}}{$\mathcal{T}, \hat{\mathbf{y}}, \mathbf{s}_{\text{last}}$}
  \Comment{Longest greedy-consistent path; if root mismatches, commit only the prefix greedy token; otherwise add a bonus token.}
  \State \Return committed tokens \(y_{t+1:t+L}\)
\EndFunction

\Statex
\Function{\textproc{UpdateCache}}{$\mathcal{K}_T, y_{t+1:t+L}, \ell$}
  \Comment{Rollback cache to length \(\ell\) then rebuild on committed tokens (crop \(\rightarrow\) forward).}
  \State \Return updated cache \(\mathcal{K}_T\)
\EndFunction
\end{algorithmic}
\end{algorithm}
