...
\begin{figure}[t]
  ...
  \caption{\textbf{Ablation progression on WikiText-2 (\(T=1500\)).} Left: throughput with speedup annotations. Right: verification efficiency, combining the number of verification iterations with per-iteration progress (\(\bar{L}\)) and acceptance rate. Dynamic Breadth \& Depth increases per-step progress and reduces iterations; History Adaptation further improves per-step progress and iteration count.}
