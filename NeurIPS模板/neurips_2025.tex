...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{../figures/ablation_bars.pdf}
  \caption{\textbf{Ablation study visualization.} Progressive improvement in (a)~throughput and (b)~speedup as components are added. Adding tree structure (D=4, B=3, \(\tau\)=0.01) provides a 32.7\% throughput gain over linear speculative decoding. Further optimizing depth and pruning (D=8, B=3, \(\tau\)=0.03) yields an additional 25.4\% improvement, reaching 221.4~tokens/s (1.79\(\times\) speedup). Percentage improvements are shown within each bar.}
  \label{fig:ablation}
\end{figure}
