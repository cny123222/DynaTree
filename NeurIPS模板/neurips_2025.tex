...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.98\linewidth]{../figures/our_approach.png}
...
For a finer-grained progressive breakdown of the three adaptive components (Fixed \(\rightarrow\) +Breadth \(\rightarrow\) +Depth \(\rightarrow\) +History) using the logged phase variants, see Appendix Table~\ref{tab:ablation-progressive-phases}.
...
\subsection{Progressive Breakdown of Adaptive Components}
\label{app:ablation-progressive}
Table~\ref{tab:ablation-progressive-phases} reports a fine-grained progressive breakdown of the three adaptive components on WikiText-2 at \(T=1500\), using the logged phase variants from our ablation runs. Here, Phase~1 corresponds to \textbf{+ Dynamic Tree Breadth}; Phase~2 adds \textbf{+ Dynamic Tree Depth}; and Phase~3 adds \textbf{+ History Adaptation}.

\begin{table}[t]
\centering
\caption{\textbf{Progressive component breakdown on WikiText-2 (\(T=1500\)).} Throughput (t/s) and speedup are computed relative to the autoregressive baseline under the same setting. \(\Delta\) vs.\ Fixed is computed relative to the fixed-tree backbone in this ablation run.}
\label{tab:ablation-progressive-phases}
\small
...
Variant & Throughput (t/s) & Speedup & \(\Delta\) vs Fixed \\
    \midrule
Fixed Tree (\(D=5,B=2\)) & 188.0\(\pm\)16.5 & 1.42\(\times\) & 0.0\% \\
Phase 1: + Dynamic Tree Breadth & 186.5\(\pm\)25.2 & 1.40\(\times\) & -0.8\% \\
Phase 2: + Dynamic Tree Depth & 213.2\(\pm\)26.1 & 1.61\(\times\) & +13.4\% \\
Phase 3: + History Adaptation & 218.5\(\pm\)22.2 & 1.65\(\times\) & +16.2\% \\
    \bottomrule
  \end{tabular}
\end{table}
