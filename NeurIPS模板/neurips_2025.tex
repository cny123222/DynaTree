...
\begin{itemize}
  ...
  \item \textbf{DynaTree (Phase 1/2/3):} Progressive variants that add confidence-aware adaptive branching (Phase~1), dynamic depth control (Phase~2), and historical adjustment (Phase~3) on top of the fixed-tree backbone.
\end{itemize}

\subsection{Main Results}
\label{main-results}
Table~\ref{tab:main-results} reports the end-to-end performance on WikiText-2 for \(T=1000\) token generation. Relative to autoregressive decoding, a fixed tree already improves throughput to 181.3~t/s (1.38\(\times\)). Incorporating confidence-aware adaptation yields further gains: Phase~2 achieves 206.0~t/s (1.57\(\times\)), and Phase~3 reaches \textbf{210.8~t/s} (\textbf{1.61\(\times\)}), a 16.3\% throughput improvement over the fixed-tree baseline. These results show that adapting tree breadth and depth to model confidence can increase per-iteration progress while maintaining greedy correctness.
...
\midrule
AR & 131.1\(\pm\)0.4 & 1.00\(\times\) & -- & 1.00 & -- \\
Fixed Tree (\(D=5,B=2\)) & 181.3\(\pm\)12.3 & 1.38\(\times\) & 80.8 & 5.65 & 5.65 \\
Phase~1: Adaptive Branch & 176.7\(\pm\)36.2 & 1.35\(\times\) & 77.9 & 5.35 & 5.45 \\
Phase~2: + Dynamic Depth & 206.0\(\pm\)29.8 & 1.57\(\times\) & 89.6 & 6.06 & 6.27 \\
\textbf{Phase~3: + History Adjust} & \textbf{210.8\(\pm\)26.5} & \textbf{1.61\(\times\)} & \textbf{94.7} & \textbf{6.49} & \textbf{6.63} \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:main-results} visualizes the throughput and speedup trends across variants. Phase~2 and Phase~3 reduce the number of verification rounds by committing longer greedy-consistent prefixes per iteration, which translates into a substantially lower per-token latency (TPOT) while preserving deterministic outputs.

\paragraph{Latency breakdown analysis.}
To understand fine-grained latency characteristics, Table~\ref{tab:latency-metrics} reports Time-To-First-Token (TTFT) and Time-Per-Output-Token (TPOT) on WikiText-2 for \(T=1000\). While all speculative variants substantially reduce TTFT relative to autoregressive decoding, the largest gains appear in TPOT: Phase~3 achieves the lowest TPOT (4.81~ms), consistent with its higher tokens-per-iteration progress.
...
\midrule
AR & 18.9\(\pm\)6.1 & 7.61\(\pm\)0.02 \\
Fixed Tree (\(D=5,B=2\)) & 12.7\(\pm\)2.7 & 5.53\(\pm\)0.38 \\
Phase~1: Adaptive Branch & 13.9\(\pm\)5.4 & 5.98\(\pm\)1.71 \\
Phase~2: + Dynamic Depth & 12.5\(\pm\)2.7 & 4.95\(\pm\)0.77 \\
\textbf{Phase~3: + History Adjust} & 12.6\(\pm\)2.7 & \textbf{4.81\(\pm\)0.63} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Throughput and speedup on WikiText-2 (\(T=1000\)).} Left: absolute throughput (tokens/s). Right: speedup relative to autoregressive decoding. Phase~3 achieves the highest throughput (210.8~t/s, 1.61\(\times\)), improving over the fixed-tree baseline (181.3~t/s, 1.38\(\times\)).}
