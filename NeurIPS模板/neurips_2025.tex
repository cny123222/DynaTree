...
\begin{itemize}
  \item We propose \textbf{DynaTree}, a training-free tree speculative decoding framework that adaptively allocates verification budget via confidence-aware adjustments to tree breadth and depth under an explicit node budget. Unlike approaches that require training auxiliary modules or architectural modifications (e.g., Medusa~\cite{medusa} or the EAGLE family~\cite{eagle,eagle2,eagle3}), DynaTree is plug-and-play for existing target--draft model pairs.
