...
We study the sensitivity of DynaTree to confidence thresholds \((\tau_h,\tau_\ell)\) and branch bounds \((B_{\min},B_{\max})\) on WikiText-2 at \(T=1000\). Table~\ref{tab:appendix-sensitivity} reports representative high-performing configurations from the released sweep logs.

\begin{table}[t]
\centering
\caption{\textbf{Parameter sensitivity on WikiText-2 (\(T=1000\)).} We vary \((\tau_h,\tau_\ell)\) and \((B_{\min},B_{\max})\) and report representative high-performing configurations. Throughput is in tokens/s (mean\(\pm\)std across prompts), and speedup is relative to AR under the same setting.}
...
\midrule
AR & 133.5\(\pm\)0.6 & 1.00\(\times\) & 0.0\% \\
\((\tau_h,\tau_\ell)=(0.9,0.4),\ (B_{\min},B_{\max})=(1,2)\) & 215.3\(\pm\)38.9 & 1.61\(\times\) & 93.8\% \\
\((\tau_h,\tau_\ell)=(0.9,0.4),\ (B_{\min},B_{\max})=(1,4)\) & 215.3\(\pm\)27.4 & 1.61\(\times\) & 95.6\% \\
\((\tau_h,\tau_\ell)=(0.8,0.3),\ (B_{\min},B_{\max})=(1,3)\) & 210.4\(\pm\)28.5 & 1.58\(\times\) & 93.3\% \\
\((\tau_h,\tau_\ell)=(0.7,0.2),\ (B_{\min},B_{\max})=(1,3)\) & 207.1\(\pm\)35.7 & 1.55\(\times\) & 91.3\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Sensitivity of DynaTree to confidence thresholds and branching bounds (WikiText-2, \(T=1000\)).} We visualize how throughput varies with the confidence thresholds \((\tau_h,\tau_\ell)\) and the branch upper bound \(B_{\max}\) (with \(B_{\min}=1\)), and summarize the throughput--acceptance tradeoff across the evaluated configurations.}
