...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../figures/main_results_bars.pdf}
  \caption{\textbf{Main results comparison.} DynaTree achieves the highest throughput (193.4 tokens/sec) and speedup (1.62×) compared to all baselines, outperforming HuggingFace assisted generation and linear speculative decoding.}
...
Table~\ref{tab:length-scaling} reports the best-performing DynaTree configuration selected per generation length, together with baseline throughput and speedup. We observe that speedup increases from short to medium lengths as verification overhead is amortized, peaking around 500 tokens in our setting.

\begin{table}[t]
\centering
\caption{Best DynaTree performance across different generation lengths.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../figures/param_sweep.pdf}
  \caption{\textbf{Parameter sweep analysis.} (a-c) Impact of tree depth $D$, branching factor $B$, and pruning threshold $\tau$ on speedup at 500-token generation. (d) Best speedup across different generation lengths with corresponding optimal configurations. (e) Average path length as a function of depth and branching factor. (f) Acceptance rate distribution across different pruning thresholds. Stars indicate optimal configurations.}
