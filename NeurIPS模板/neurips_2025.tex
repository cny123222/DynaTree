...
\label{main-results}
Table~\ref{tab:main-results} reports end-to-end throughput on WikiText-2 and PG-19 for \(T=1500\). DynaTree achieves the highest throughput on both datasets, improving over autoregressive decoding, linear speculative decoding, and a fixed-tree baseline. The gains over a tuned static tree (e.g., 218.5~t/s vs.\ 188.0~t/s on WikiText-2) suggest that allocating verification budget according to local model uncertainty improves the draft--verify trade-off beyond a fixed configuration.
...
& Accept. & \(\bar{L}\) (tok/iter) & \(\bar{\ell}\) (path) & \#Iter. & Accept. & \(\bar{L}\) (tok/iter) & \(\bar{\ell}\) (path) & \#Iter. \\
\midrule
AR & -- & 1.00 & -- & 1500 & -- & 1.00 & -- & 1500 \\
Linear Spec (\(K=5\)) & 97.7\% & 4.84 & 4.88 & 310 & 92.5\% & 4.56 & 4.63 & 329 \\
Fixed Tree (\(D=5,B=2\)) & 82.6\% & 5.75 & 5.78 & 261 & 82.9\% & 5.75 & 5.80 & 261 \\
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{../figures/ablation_progression.pdf}
...
We evaluate how throughput varies with generation length \(T\) on WikiText-2, comparing autoregressive decoding, linear speculative decoding, a fixed tree baseline, and DynaTree. Figure~\ref{fig:length-scaling} summarizes throughput trends across \(T\). Figure~\ref{fig:length-scaling-efficiency} further decomposes these trends via \(\bar{L}\), \(\bar{\ell}\), and acceptance rate, illustrating how per-iteration progress and verification success vary with generation length.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{../figures/length_scaling_efficiency.pdf}
  \caption{\textbf{Verification efficiency across generation lengths on WikiText-2.} We report tokens per iteration (\(\bar{L}\)), average committed path length (\(\bar{\ell}\)), and acceptance rate as functions of \(T\) to explain the throughput trends.}
  \label{fig:length-scaling-efficiency}
\end{figure}
...
We study the sensitivity of DynaTree to confidence thresholds \((\tau_h,\tau_\ell)\) and branch bounds \((B_{\min},B_{\max})\) on WikiText-2 at \(T=1500\). The released logs cover a coarse grid of representative configurations; Table~\ref{tab:appendix-sensitivity} reports these settings and Figure~\ref{fig:parameter-sensitivity} visualizes the resulting throughput trends.

\begin{table}[t]
\centering
\caption{\textbf{Parameter sensitivity on WikiText-2 (\(T=1500\)).} We vary \((\tau_h,\tau_\ell)\) and \((B_{\min},B_{\max})\) and report released configurations from the sweep logs. Throughput is in tokens/s (mean\(\pm\)std across prompts), and speedup is computed relative to AR under the same setting. Acceptance rate (Accept.) can slightly exceed 100\% due to the bonus-token convention in tree verification.}
...
\midrule
AR & 132.9\(\pm\)0.5 & 1.00\(\times\) & 0.0\% \\
\((\tau_h,\tau_\ell)=(0.9,0.4),\ (B_{\min},B_{\max})=(1,3)\) & 217.8\(\pm\)22.2 & 1.64\(\times\) & 102.2\% \\
\((\tau_h,\tau_\ell)=(0.9,0.4),\ (B_{\min},B_{\max})=(1,2)\) & 217.8\(\pm\)34.6 & 1.64\(\times\) & 100.7\% \\
\((\tau_h,\tau_\ell)=(0.9,0.4),\ (B_{\min},B_{\max})=(1,4)\) & 217.7\(\pm\)22.9 & 1.64\(\times\) & 102.6\% \\
\((\tau_h,\tau_\ell)=(0.7,0.2),\ (B_{\min},B_{\max})=(1,3)\) & 211.9\(\pm\)34.1 & 1.59\(\times\) & 97.8\% \\
\((\tau_h,\tau_\ell)=(0.8,0.3),\ (B_{\min},B_{\max})=(1,3)\) & 201.6\(\pm\)24.7 & 1.52\(\times\) & 100.1\% \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Sensitivity of DynaTree to confidence thresholds and branching bounds (WikiText-2, \(T=1500\)).} We visualize how throughput varies with the confidence thresholds \((\tau_h,\tau_\ell)\) and the branch upper bound \(B_{\max}\) (with \(B_{\min}=1\)), and summarize the throughput--acceptance tradeoff across the evaluated configurations.}
...
\begin{figure}[t]
  ...
  \caption{\textbf{Fixed-tree hyperparameter sweep under the paper protocol (WikiText-2, \(T=1500\)).} Speedup is computed relative to autoregressive decoding under the same setting. Stars indicate the best configuration observed in the sweep.}
